...
# 设置模型文件夹路径
models_dir = "D:/cursor/Project/SNS_Project-main/Models"  # 确保路径正确
            ...
            try:
                ...
                if data["type"] == "predict":
                    if "date" not in data:
                        await websocket.send(json.dumps({
                            "status": "error",
                            "message": "Missing 'date' field in request"
                        }))
                        continue
...
async def main():
    server = await websockets.serve(handle_websocket, "0.0.0.0", 8765)  # 允许外部访问
